...
async def start(update: Update, context: CallbackContext) -> None:
    ...
    await update.message.reply_text(
        f"Добро пожаловать! Нажмите кнопку 'День 1', чтобы приступить к изучению.",
...
async def ask_video_count(update: Update, context: CallbackContext) -> None:
    ...
    # Попросим администратора ввести количество видео
    context.user_data['day'] = day
    await update.message.reply_text(f"Сколько видео хотите добавить для дня {day}?")
...
async def add_video_links(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        await update.message.reply_text("Введите корректное количество видео.")
        return

    context.user_data['video_count'] = video_count
    await update.message.reply_text(f"Добавьте {video_count} видео для дня {day}.")

async def save_video_link(update: Update, context: CallbackContext) -> None:
    if 'video_count' not in context.user_data:
        await update.message.reply_text("Сначала выберите количество видео.")
        return

    day = context.user_data['day']
    video_count = context.user_data['video_count']

    if day not in video_links_by_day:
        video_links_by_day[day] = []

    if len(video_links_by_day[day]) < video_count:
        video_links_by_day[day].append(update.message.text)
        remaining = video_count - len(video_links_by_day[day])
        if remaining > 0:
            await update.message.reply_text(f"Добавьте ещё {remaining} видео.")
        else:
            await update.message.reply_text(f"Все {video_count} видео для дня {day} добавлены.")
    else:
        await update.message.reply_text(f"Для дня {day} уже добавлено {video_count} видео.")
...
async def start_day(update: Update, context: CallbackContext) -> None:
    ...
    last_day, next_available_time = user_progress.get(user_id, (0, datetime.now()))
    
    # Доступность первого дня сразу
    if last_day == 0:
        last_day = 1
...
def main() -> None:
    ...
    application.add_handler(MessageHandler(filters.Text, add_video_links))
    application.add_handler(MessageHandler(filters.Text & filters.Regex(r'http'), save_video_link))
